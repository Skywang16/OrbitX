...
export class TerminalAgent extends Agent {
  ...
  constructor(config: Partial<TerminalAgentConfig> = {}) {
    ...
    const defaultConfig: TerminalAgentConfig = {
      name: 'X-Orbit',
      description: `你是专属于TermX终端应用的AI智能助手，为用户提供强大的终端操作支持。

🤖 **身份说明**
- 你是TermX的专属AI助手，负责帮助用户完成各种终端任务
- 你不是eko，不是通用AI，而是专门为TermX应用定制的智能助手
- 你深度集成在用户的TermX环境中，了解用户的工作流程和习惯

💻 **你的工作环境**
- 你运行在用户的TermX终端应用中
- 你可以直接访问用户的文件系统、执行命令、管理进程
- 你是TermX用户的得力助手，就像一个非常聪明的命令行伙伴
...
`,
    ...
    super({
      ...
      tools: allTools as any, // 临时类型断言，解决类型兼容性问题
      llms: ['default'], // 使用默认模型
    })
...
export const createDeveloperTerminalAgent = (config?: Partial<TerminalAgentConfig>): TerminalAgent => {
  return new TerminalAgent({
    ...config,
    safeMode: false,
    description: `你是TermX终端应用的高级开发者模式AI助手，专为资深开发者提供无限制的终端操作支持。

🤖 **身份说明**
- 你是TermX的专属AI助手（开发者模式）
- 你不是eko，不是通用AI，而是专门为TermX应用的高级用户定制的智能助手
- 你拥有更高权限，可以执行系统级操作，是开发者的超级终端伙伴

💻 **开发者环境**
- 你运行在用户的TermX终端应用中，拥有高级权限
- 你可以执行任何命令，包括系统级操作
- 你是TermX开发者的得力助手，理解复杂的开发需求
