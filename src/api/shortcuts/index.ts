...
export class ShortcutApi {
    ...
    try {
      ...
      if (options.validate !== false) {
        ...
        if (!validation.is_valid) {
          throw new ShortcutApiError(
            `快捷键配置验证失败: ${validation.errors.map(e => e.message).join(', ')}`
          );
        }
      }
      ...
      if (options.checkConflicts) {
        ...
        if (conflicts.has_conflicts) {
          throw new ShortcutApiError(
            `快捷键配置存在冲突: ${conflicts.conflicts.length} 个冲突`
          );
        }
      }
  ...
  static async addShortcut(
    ...
    try {
      ...
      if (options.validate !== false) {
        ...
        if (!validation.is_valid) {
          throw new ShortcutApiError(
            `快捷键验证失败: ${validation.errors.map(e => e.message).join(', ')}`
          );
        }
      }
      ...
      const searchInCategory = (
        ...
        shortcuts.forEach((binding, index) => {
          ...
          if (options.query) {
            ...
            const modifierMatches = binding.modifiers.some(m => 
              m.toLowerCase().includes(query)
            );
            const actionMatches = typeof binding.action === 'string' 
          ...
          if (options.action && matches) {
            const actionStr = typeof binding.action === 'string'
              ? binding.action
              : binding.action.action_type;
  ...
  static async exportConfig(
    options: ShortcutImportExportOptions = {}
  ): Promise<string> {
    try {
      const config = await this.getConfig();
  ...
  static async importConfig(
    ...
    try {
      ...
      if (!validation.is_valid) {
        throw new ShortcutApiError(
          `导入的配置验证失败: ${validation.errors.map(e => e.message).join(', ')}`
        );
      }
