 ...
 *
 * 提供Shell Integration API，让任何组件都能使用
 */
...
class ShellIntegrationService {
  ...
  async executeCommand(terminalId: string, command: string, timeout = 30000): Promise<CommandResult> {
    return new Promise((resolve, reject) => {
      ...
      // 执行命令
      this.writeToTerminal().catch(error => {
        cleanup()
...
export function useShellIntegrationAPI(terminalId: string) {
  return {
    ...
    getCurrentWorkingDirectory: () => shellIntegrationService.getCurrentWorkingDirectory(terminalId),

    getCommandHistory: () => shellIntegrationService.getCommandHistory(terminalId),

    onEvent: (listener: ShellIntegrationListener) => shellIntegrationService.onEvent(listener),
  }
}
