...
export class EkoMemory {
  ...
  public async import(data: { messages: EkoMessage[]; config?: MemoryConfig }): Promise<void> {
  ...
  public removeMessageById(id: string, removeToNextUserMessages: boolean = true): string[] | undefined {
  ...
  protected async manageCapacity(): Promise<void> {
    ...
    if (this.enableCompression && this.messages.length > this.compressionThreshold) {
      // compress messages
      for (let i = 0; i < this.messages.length; i++) {
        const message = this.messages[i]
    ...
    while (this.getEstimatedTokens(true) > this.maxTokens && this.messages.length > 0) {
      this.messages.shift()
    }
