    ...
    return [
      {
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() => this.create_session(agentContext, args.exec_dir as string))
        },
      },
      {
        name: 'shell_exec',
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() =>
            this.shell_exec(agentContext, args.session_id as string, args.command as string)
          )
        },
      },
      {
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() => this.close_session(agentContext, args.session_id as string))
        },
      },
    ]
  }
