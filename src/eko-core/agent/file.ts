...
export default abstract class BaseFileAgent extends Agent {
  ...
  protected async do_file_read(agentContext: AgentContext, path: string): Promise<string> {
    ...
    return [
      {
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() => this.file_list(agentContext, args.path as string))
        },
      },
      {
        name: 'file_read',
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() => this.do_file_read(agentContext, args.path as string))
        },
      },
      {
        name: 'file_write',
      ...
      },
      {
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() =>
            this.file_str_replace(agentContext, args.path as string, args.old_str as string, args.new_str as string)
          )
        },
      },
      {
        ...
        execute: async (args: Record<string, unknown>, agentContext: AgentContext): Promise<ToolResult> => {
          return await this.callInnerTool(() =>
            this.file_find_by_name(agentContext, args.path as string, args.glob as string)
          )
        },
      },
    ]
