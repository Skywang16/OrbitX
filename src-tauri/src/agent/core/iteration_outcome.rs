...
impl IterationOutcome {
    ...
    pub fn get_output_for_persistence(&self) -> Option<String> {
        match self {
            Self::Complete { thinking, output } => {
                output.clone().or_else(|| thinking.clone())
            }
            _ => None,
        }
    }
    ...
    pub fn get_full_content(&self) -> Option<(Option<String>, Option<String>)> {
        match self {
            Self::Complete { thinking, output } => {
                Some((thinking.clone(), output.clone()))
            }
            _ => None,
        }
    }
    ...
    pub fn has_content(&self) -> bool {
        match self {
            Self::Complete { thinking, output } => {
                thinking.is_some() || output.is_some()
            }
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            Self::Complete { thinking, output } => {
                match (thinking.is_some(), output.is_some()) {
                    (true, true) => "完成（有思考和输出）",
                    (true, false) => "完成（仅有思考）",
                    (false, true) => "完成（仅有输出）",
                    (false, false) => "完成（无内容）",
...
mod tests {
    ...
    fn test_should_continue() {
        let continue_outcome = IterationOutcome::ContinueWithTools {
            tool_calls: vec![],
        };
    ...
    fn test_get_output_for_persistence() {
        ...
        assert_eq!(outcome1.get_output_for_persistence(), Some("输出".to_string()));
        ...
        assert_eq!(outcome2.get_output_for_persistence(), Some("思考".to_string()));
