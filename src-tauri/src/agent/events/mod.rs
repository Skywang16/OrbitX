...
pub struct TaskProgressEvent {
    pub payload: TaskProgressPayload,
    pub sequence: u64,
    pub source: String,
...
impl TaskProgressEvent {
    /// 创建新的进度事件
    pub fn new(payload: TaskProgressPayload, sequence: u64) -> Self {
        Self {
            payload,
            sequence,
            source: "task_executor".to_string(),
            created_at: Utc::now(),
        }
    }

    /// 创建带自定义源的进度事件
    pub fn with_source(payload: TaskProgressPayload, sequence: u64, source: String) -> Self {
        Self {
            payload,
            sequence,
            source,
