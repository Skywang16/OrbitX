...
pub async fn get_terminal_context() -> Result<serde_json::Value, String> {
    ...
    // 安全的环境变量，不包含敏感信息
    let safe_env_vars = [
        "PATH", "HOME", "USER", "SHELL", "PWD", "TERM", "LANG", "LC_ALL", "TMPDIR", "LOGNAME",
    ];
    ...
    system_info.insert("os".to_string(), env::consts::OS.to_string());
    system_info.insert("arch".to_string(), env::consts::ARCH.to_string());
    system_info.insert("family".to_string(), env::consts::FAMILY.to_string());
    ...
    let result = serde_json::to_value(context).map_err(|e| e.to_string())?;

    debug!(
        "终端上下文获取完成: workingDirectory={}",
        result
