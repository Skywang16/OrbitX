...
use crate::ai::{create_context_manager, create_context_manager_with_config, ContextConfig};
use crate::storage::database::{DatabaseManager, DatabaseOptions};
use crate::storage::paths::StoragePaths;
use crate::storage::repositories::{Conversation, Message, RepositoryManager};
...
async fn test_build_context_without_compression() {
    ...
    assert!(!result.compressed, "should not be compressed for small history");
...
async fn test_build_context_with_compression_path_executes() {
    ...
    for i in 0..20 {
        if i % 2 == 0 {
            ...
            data.push(("assistant", "收到，我们会对早期消息进行摘要压缩并保留最近消息。"));
        }
    }
