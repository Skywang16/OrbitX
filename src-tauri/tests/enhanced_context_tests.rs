...
mod tests {
    ...
    fn create_tool_message(id: i64, conv_id: i64, tool_name: &str) -> Message {
        let steps_json = format!(
            r#"[{{"type":"tool_use","toolExecution":{{"name":"{}","params":{{}},"result":"success"}}}}]"#,
            tool_name
        );
        Message {
            id: Some(id),
    ...
    fn test_message_scorer_length_optimization() {
        ...
        let optimal_msg = create_test_message(
            2,
            100,
            "user",
            "This is an optimal length message for scoring purposes.",
        ); // ~300 chars
    ...
    fn test_hybrid_strategy_deduplication() {
        ...
        // 重复消息应该被去除
        let duplicate_count = compressed
            .iter()
    ...
    fn test_loop_detector_exact_duplicates() {
        ...
        assert!(
            filtered
                .iter()
                .filter(|m| m.content == identical_content)
                .count()
                == 1
        );
    }
